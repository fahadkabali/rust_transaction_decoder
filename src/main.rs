use clap::Parser;

#[derive(Parser)]
#[command(name = "Transaction decoder")]
#[command(version = "0.1.0")]
#[command(author = "Fahad")]
#[command(about = "Decode a Bitcoin transaction from its raw hex format", long_about = None)]

struct Cli{
    #[arg(
        required = true,
        help = "(String, required)The raw transaction in hex format"
    )]
    transaction_hex: String,
}


fn main() {
    // let transaction_hex = "";
    let cli = Cli::parse();
    let transaction_hex = cli.transaction_hex;
    match transaction_decoder::decode(transaction_hex) {
        Ok(transaction_json) => println!("Transaction: {}", transaction_json),
        Err(e) => eprintln!("Error: {}", e),
    }
}

     